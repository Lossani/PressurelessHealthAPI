...
def calculate_challenge_requirements(user: User):
    challenges = Challenge.objects.filter(enabled = True).exclude(
        Q(repeatable = False) &
        Q(Exists(ChallengeHistory.objects.filter(user = user, challenge_id = OuterRef('id'), end_date__isnull = False, succeeded = True)))
    ).prefetch_related(
        Prefetch('challengehistory_set',
                 queryset= ChallengeHistory.objects.filter(user = user, end_date__isnull = True).order_by('-pk')[:1],
                 to_attr='histories'
                 )
    )
    # .annotate(
